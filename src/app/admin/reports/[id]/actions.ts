...
export async function getEventStatistics(eventId: string) {
  ...
  const logs = await prisma.attendanceLog.findMany({
    ...
    include: {
      user: {
        select: { yearLevel: true, section: true, department: { select: { code: true } } }
      }
    }
  })
  ...
  logs.forEach(log => {
    ...
    const dept = log.user.department?.code || "Unassigned"
  ...
  return {
    ...
    byDepartment: Object.entries(deptStats).map(([name, value]) => ({ name, value })).sort((a, b) => b.value - a.value)
  }
}

export async function getEventAttendees(eventId: string, page = 1, pageSize = 10, yearLevel = "ALL", section = "ALL", departmentId = "ALL", search = "") {
  const session = await getSession()
  if (!session || session.role !== "SUPER_ADMIN") {
    throw new Error("Unauthorized")
  }
