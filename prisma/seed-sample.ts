...
async function main() {
  const departments = [
    { code: "BSIT", name: "BS Information Technology", color: "#1A3A8F", lightBg: "#EEF2FF" },
    { code: "BEED/BSED", name: "Bachelor of Elementary/Secondary Education", color: "#DC2626", lightBg: "#FEE2E2" },
    { code: "BSHM", name: "BS Hospitality Management", color: "#0F6E56", lightBg: "#E1F5EE" },
    { code: "BSTM", name: "BS Tourism Management", color: "#D97706", lightBg: "#FEF3C7" },
    { code: "BSBA", name: "BS Business Administration", color: "#0891B2", lightBg: "#E0F2FE" },
    { code: "BSCRIM", name: "BS Criminology", color: "#7C3AED", lightBg: "#EDE9FE" },
    { code: "BSMARINE", name: "BS Marine Transportation", color: "#0284C7", lightBg: "#E0F2FE" },
  ]

  const createdDepartments: Record<string, string> = {}
  for (const dept of departments) {
    const created = await prisma.department.findUnique({ where: { code: dept.code } })
    if (created) createdDepartments[dept.code] = created.id
  }

  const hashedPasswordAdmin = await bcrypt.hash("password123", 12)
  const hashedPasswordStudent = await bcrypt.hash("Student@123", 12)

  const superAdmin = await prisma.user.findUnique({ where: { email: "admin@gmail.com" } })

  // Create BSIT Dept Admin
